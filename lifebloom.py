...
async def thread_completion(thread_state: dict, actions_response: List[Dict]=[]):
    ...
    while current_completion_i <= MAX_INTERNAL_COMPLETIONS:
        actions_to_take, thread_state = await workflow_executor.internal_thread_completion(thread_state)
        if actions_to_take or thread_state["is_exit"]:
            return actions_to_take, thread_state
        else:
            current_completion_i += 1
            
    raise ExitHandlerError("Internal completions exceeded the maximum number of completions allowed.")

# ...Lots more functions for auth, configurations, LLM chat completions, internal function calls to ontology, etc.


def initialize_thread_state(thread_input: dict, workflow_name: str, mode: str = "precision", persona_object: dict=None) -> dict:
    """
...
async def execute_simulation(scenario_definition, original_input, actions_context, action, actions_path="function.action"):
    workflow_executor = WorkflowExecutor()
    ...
    try:
        ...
        action_attributes = workflow_executor.filter_taxonomy_objects(workflow_executor.ontology, actions_path,attribute_filter)[0]
...
async def simulate_scenario(workflow_name, scenario_description, persona_object):
    
    workflow_executor = WorkflowExecutor()
        
    # GET SCENARIO DEFINITION
    scenario_defintion = workflow_executor.ontology["examples"][scenario_description]
